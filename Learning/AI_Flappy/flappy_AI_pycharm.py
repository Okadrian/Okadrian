...
BIRD_IMG S =[pygame.transform.scale2x(pygame.image.load(os.path.join("img" ,"bird1.png")))
...
class Bird:
    ...
    def move(self):
        ...
        d = self.ve l *self.tick_count + 1. 5 *self.tick_coun t* *2
...
def draw_window(win, birds, pipes, base, score, gen):
    ...
    win.blit(text, (WIN_WIDTH - 40 - text.get_width(), 10))

    base.draw(win)

    GEN_text = STAT_FONT.render("GEN" + str(gen), 1, (255, 255, 255))

    win.blit(GEN_text, (100 - GEN_text.get_width(), 10))
...
class Pipe:
    ...
    def set_height(self):
        ...
        self.top = self.height - self.PIPE_TOP.get_height() - 100
...
def main(genomes, config):  # This is main
...
def run(config_path):
    p = neat.Population(neat.DefaultGenome, neat.DefaultReproduction, neat.DefaultSpeciesSet, neat.DefaultStagnation,
                        config_path)
